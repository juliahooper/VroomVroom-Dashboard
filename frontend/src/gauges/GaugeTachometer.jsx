/**
 * Classic car-style RPM tachometer: circular dial, dark face, white scale,
 * orange needle. Green / yellow / red zones from backend danger/warning thresholds.
 */
export default function GaugeTachometer({
  value = 0,
  max = 400,
  dangerThreshold = 300,
  ...
  return (
    <div className="boat-gauge boat-gauge-tach boat-gauge-tach--classic">
      <div className="boat-gauge__face boat-gauge__face--dark">
        <div className="boat-gauge__dial">
          <svg viewBox="0 0 100 100" className="boat-gauge__svg" aria-hidden>
            <defs>
              <linearGradient id="tach-face-shine" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor="#2a2a2a" />
                <stop offset="100%" stopColor="#0f0f0f" />
              </linearGradient>
              <clipPath id="clip-tach">
            ...
            {/* Dark face */}
            <circle cx={cx} cy={cy} r={r} fill="url(#tach-face-shine)" />
            {/* Filled threshold bands: green (good), yellow (warning), red (danger) – Performance Gauge style */}
            <g clipPath="url(#clip-tach)">
          ...
          {majorTicks.map((num) => {
            ...
            return (
              <text
                ...
                className="boat-gauge-tach__number"
          ...
          <line
            ...
            strokeWidth="2"
            strokeLinecap="round"
            style={{ transition: 'all 0.3s ease-out', filter: 'drop-shadow(0 0 1px rgba(0,0,0,0.5))' }}
          />
          <circle cx={cx} cy={cy} r="5" fill="#1a1a1a" stroke="#333" strokeWidth="1" />
          </svg>
        </div>
        <div className="boat-gauge__value boat-gauge-tach__value">{Math.round(value)}</div>
      </div>
    </div>
  )
}
