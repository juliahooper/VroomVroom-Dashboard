...
When you use "with closing(socket.socket(...)) as sock:", Python guarantees it will
call sock.close() when the block ends—even if an error happens—so we don't leave
sockets open (which can cause "address in use" / TIME_WAIT).
"""
